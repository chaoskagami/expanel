...
#define HOME_THEME_PATH "/.expanel/themes"
#define SHARE_THEME_PATH PREFIX "/share/expanel/themes"
...
static const char *theme = "default";
static const char *version = "EXPanel " BMPANEL_VERSION "\nDerived from BMPanel 0.9.27\n";
static const char *usage = "usage: expanel [options] THEME\n\t-h (--help)\t\t\tPrints usage info. \n\t-v (--version)\t\t\tPrints version info. \n\t-l (--list-themes)\t\tList available themes. \n\t-b (--bmpanel-compat)\t\tUses bmpanel themes in compat mode.\n";
...
static int bmpanel_compat = FALSE;
...
static void handle_selection_clear(XSelectionClearEvent *e)
{
	...
	/* if expanel lost selection, try to get it again */
...
{
	if(!bmpanel_compat) {
		printf("[NOTICE] EXPanel specific functionality NYI. Falling back to BMPanel compat.\n");
		bmpanel_compat = TRUE;
	}
	if(bmpanel_compat) {
		char dirbuf[4096];
...
static void parse_args(int argc, char **argv)
{
	...
	for (i = 1; i < argc; i++) {
		...
		if (!strcmp(arg, "--list-themes") || !strcmp(arg, "-t")) {
			list_themes();
			fflush(stdout);
			exit(0);
		}
		if (!strcmp(arg, "--bmpanel-theme") || !strcmp(arg, "-b")) {
			bmpanel_compat = TRUE;
		}
		break;
	}
...
int main(int argc, char **argv)
{
	...
	LOG_MESSAGE("starting expanel with theme: %s", theme);
